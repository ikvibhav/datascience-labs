...
### Execution Inputs
The user will provide instructions using the following syntax:
* **Target File**: `[path/to/source_file.py]`
* **Test File Output**: `[tests/path/to/test_source_file.py]`

If the user only specifies the **Target File**, automatically derive the **Test File Output** path by:
1. Mirroring the source directory structure inside a root `tests/` directory.
2. Appending the prefix `test_` to the filename.
*Example: `src/utils/parser.py` becomes `tests/utils/test_parser.py`.*

### Existing Tests
If the target file already has an associated test file, append new tests to it instead of creating a new file. Ensure that the new tests are integrated logically with existing ones.
